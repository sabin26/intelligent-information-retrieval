...
import { Publication, GroundingMetadata, ClassificationResult, DocCategory } from '../types';
...
const classificationSchema = {
    ...
    properties: {
        category: {
            type: Type.STRING,
            enum: [DocCategory.Politics, DocCategory.Business, DocCategory.Health],
...
export const searchPublications = async (query: string): Promise<{ publications: Publication[], groundingMetadata?: GroundingMetadata }> => {
    try {
        const prompt = `
        ...
        if (responseText) {
            try {
                ...
                throw new Error("The model returned an unexpected response format. Could not parse publication data.");
            }
        }
        ...
        throw new Error("Failed to fetch publication data from the Gemini API. Please check the console for details.");
    }
};
...
export const classifyDocument = async (documentText: string): Promise<ClassificationResult> => {
    ...
    try {
        ...
        throw new Error("Failed to classify document using the Gemini API. Please check the console for details.");
    }
};
