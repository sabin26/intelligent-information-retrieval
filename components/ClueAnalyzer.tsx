...
const Task2_DocumentClassifier: React.FC<Task2Props> = ({ onComplete }) => {
  ...
  const classifyText = (text: string): ClassificationResult => {
    ...
    let confidence = Math.random() * (0.65 - 0.40) + 0.40;

    if (/\b(politics|government|election|bill|senate|law)\b/.test(lowerText)) {
      category = DocCategory.Politics;
      confidence = Math.random() * (0.98 - 0.80) + 0.80;
    } else if (/\b(business|market|stock|profits|investment|corporate|economic)\b/.test(lowerText)) {
      category = DocCategory.Business;
      confidence = Math.random() * (0.97 - 0.82) + 0.82;
    } else if (/\b(health|doctor|wellness|cardiovascular|medical|hospital)\b/.test(lowerText)) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm p-6 sm:p-8 rounded-lg shadow-lg border border-slate-200/80">
      <h3 className="text-xl font-bold text-slate-800 font-display tracking-wide">Task 2: Document Classification</h3>
      <p className="text-sm text-slate-600 mt-1 mb-6">Enter a document (e.g., a sentence or paragraph) to classify it into one of the existing categories: Politics, Business, or Health.</p>
        ...
        {result && (
          ...
          >
            ...
            <div className="text-center bg-slate-50 p-6 rounded-lg">
                ...
                {result.confidence && (
                    <div className="mt-4">
                        <p className="text-xs text-slate-500">Classification Confidence</p>
                        <p className="font-bold text-lg text-slate-700">{(result.confidence * 100).toFixed(1)}%</p>
                    </div>
                )}
            </div>
          </motion.div>
